...
class Power(object):
    ...
    def __getitem__(self, name):
        if name in self.map:
            ...
            value = any([v < 10 for v in Utility.filter(self.data, np.mean, 7)])
        elif name == "is_stop_rel":
            value = any([v < 0.1 * self.ave for v in Utility.filter(self.data, np.mean, 7)])
    ...
    def plot(self, path):
        ...
        plt.plot(self.data[365:])
        name = str(self.rank) + ":" + str(self.id)
        plt.savefig("./images/"+path+"/"+name+".jpg")
        plt.close()

    def similar(self):
        r = 3
        s1 = self.norm(self.data[:250])
        s2 = self.norm(self.data[365:])
        return Utility.DTWDistance(s1, s2, r) < 5
...
class Powers(object):
    ...
    def __init__(self, file):
        ...
        if os.path.exists(pfile):
            ...
            self.data = np.array([list(user.power_consumption) for user in users])
            print self.data.shape
            with open(pfile, "wb") as f:
                pickle.dump(self.data, f)
        self.train = self.data[:, :609].astype(np.float)
        self.test  = self.data[:, 609:].astype(np.float)
        # split train/test
    ...
    def classify(self, p):
        for name in ["is_nodata", "is_big", "is_stop_abs", "is_stop_rel", "is_year_similar"]:
            if p[name]:
                p.plot(name)
                break
            p.plot("other")
